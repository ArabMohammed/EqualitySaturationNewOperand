...
impl<L: Language, N: Analysis<L>> EGraph<L, N> {
    ...
    /********************************************************************************************/
    /********************************************************************************************/
    pub fn find_eclass_by_id(&mut self, id: Id) -> Option< &mut EClass<L, N::Data>> {
        self.classes_mut()
            .find(|eclass| eclass.id == id) 
    }
    
    pub fn empty_eclass_nodes(&mut self, id: Id) {
        /******************************************************************************/
        let (length, last_node) = {
            if let Some(found_eclass) = self.find_eclass_by_id(id) {
                let length: usize = found_eclass.nodes.len();
                // we need to verify that none of the eclass nodes has as child the same eclass
                let mut possible = true ;
                for node in &found_eclass.nodes{
                ...
                /***********************************************/
                if length > 1 && possible {
                    if let Some(last_node) = found_eclass.nodes.last().cloned() {
                        (length, Some(last_node))
                    } else {
                        (length, None)
                    }
                } else {
        ...
        // Now, perform the retain operation
        if let Some(last_node) = last_node {
                self.classes_mut().for_each(|eclass| {
                        eclass.parents.retain(|parent| {
                            (parent.1 != id || (parent.1 == id && parent.0 == last_node))
                        });
                });
                // Finally, clear the nodes up to length - 1
                if let Some(found_eclass) = self.find_eclass_by_id(id) {
                    found_eclass.clear_nodes(length-1);
                }
        }
        /******************************************************************************
        let (length, first_node) = {
            if let Some(found_eclass) = self.find_eclass_by_id(id) {
                let length: usize = found_eclass.nodes.len();
                // we need to verify that none of the eclass nodes has as child the first node 
                let mut possible = true ;
                for node in &found_eclass.nodes{
                    if node.children().contains(&id) {
                        possible=false;
                ...
                if length > 2 && possible {
                    if let Some(first_node) = found_eclass.nodes.first().cloned() {
                        (length, Some(first_node))
                    } else {
                        (length, None)
                    }
                } else {
        ...
        if let Some(first_node) = first_node {
                self.classes_mut().for_each(|eclass| {
                        eclass.parents.retain(|parent| {
                            (parent.1 != id || (parent.1 == id && parent.0 != first_node))
                        });
                });
                // Finally, clear the nodes up to length - 1
                if let Some(found_eclass) = self.find_eclass_by_id(id) {
                    println!("clearing eclass nodes {:?}",found_eclass);
                    found_eclass.clear_nodes(1);
                }
        }
        ****************************/
        /************************************************************************/
        ...
        /********************************************************************************/
        /* match self.find_eclass_by_id(id) {
            Some(found_eclass) => {
                let length : usize = found_eclass.nodes.len() ;
                if length > 1 {
                    if let Some(last_node) = found_eclass.nodes.last().cloned() {
