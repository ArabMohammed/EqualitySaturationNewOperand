...
use smallvec::SmallVec;     // Import SmallVec
    ...
    IterData: IterationData<L, N>,
{
        ...
        N: 'a,
    {
        ...
        check_rules(&rules);
        // here we can divide rules into two categories some are applied after 
        // saturation 
    ...
    fn run_one(&mut self, rules: &[&Rewrite<L, N>]) -> RunnerResult<()> {
        ...
        let info = applied.is_empty() ;
        let info1 = self.scheduler.can_stop(i); 
        ...
        if saturated {
            println!("Reaching saturation : {} {}",info,info1);
    ...
    /****************************************************************************
    fn run_one_after_saturation(&mut self, rules: &[&Rewrite<L, N>]){
        let i = 0 ;
        let egraph_nodes = self.egraph.total_size();
        let egraph_classes = self.egraph.number_of_classes();
        let mut matches = Vec::new();
...
***********************/
    fn search_rewrite(
        ...
        if let Some(limit) = self.stats.get_mut(rewrite.name()) {
            ...
            let max_substitution_eclass = 50 ;
            let mut matchess = rewrite.search(egraph); // return  Vec<SearchMatches> 
            let matches = limit_subs_size(matchess,max_substitution_eclass);
            /*
...
fn limit_subs_size(mut matches: Vec<SearchMatches>, max_subs_size: usize) -> Vec<SearchMatches> {
    let mut rng = thread_rng();
    /***************************************/
    let max_nb_subs : usize = 20000 ;             
    let total_len: usize = matches.iter().map(|m| m.substs.len()).sum();
    let max_nb_eclasses = max_nb_subs / max_subs_size ;

    if total_len > max_nb_subs && matches.len() > max_nb_eclasses {
        matches.shuffle(&mut rng);
        matches.truncate(max_nb_eclasses);
    }
    /**************************************/
    matches.into_iter().map(|mut match_item| {
        // Shuffle the `substs` vector
        let mut substs = match_item.substs.clone();    let mut enable_rotation : bool = true ;

        substs.shuffle(&mut rng);
        
        // Limit the size of the `substs` vector
        if substs.len() > max_subs_size {
            substs.truncate(max_subs_size);
        }
        
        // Return the new `SearchMatches` with limited `substs`
        SearchMatches {
            eclass: match_item.eclass,
            substs,
        }
    }).collect()
}
